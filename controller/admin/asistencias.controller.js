...
const { registro_asistencia: Asistencia, empleados: Empleados } = models;
...
// consultar asistencias con filtros por empleado, fecha o rango
asistencia.get('/consultar', [
  ...
  try {
    ...
    const asistencias = await Asistencia.findAll({ where: whereClause });
    res.json({ asistencias });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Error al consultar asistencias', detail: error.message });
  }
});
